...
@bot.command() # Displays memers pfp
async def pfp(ctx, member: Member = None):
 if not member:
  member = ctx.author
 await ctx.send(member.avatar_url)

@bot.command(name="id") # Returns your ID
async def _id(ctx):
    author = ctx.message.author
    await ctx.send('Your ID is: ' + str(author.id))

@bot.command() # Kill command
...
@bot.command(pass_context=True) #Put an MP3 file in the MP3 folder and edit SONG.mp3 to your song name
async def mp3(ctx):
    await ctx.send(file=discord.File(r'mp3/SONG.mp3'))
...
async def hi(ctx):
    ...
    elif author.id == USERID: # Edit USERID and YOURMESSAGEHERE to have a custom message for a persion
        await ctx.send("YOURMESSAGEHERE")
    else:
        await ctx.send("Hello")

@bot.command(pass_context=True) #Put an MP$ file in the MP$ folder and edit YOURVIDEO.mp4 to your song name
async def mp4(ctx):
    await ctx.send(file=discord.File(r'mp4/YOURVIDEO.mp4'))
...
@bot.command(pass_context=True) # Sends a picture
async def image(ctx):
    await ctx.send(file=discord.File(r'img/IMAGE.png'))


@bot.command() # Edit meme.txt
async def meme(ctx):
...
@bot.command()
async def info(ctx):
    embed=discord.Embed(title="Bot Info", color=0x800080)
    ...
    embed.add_field(name="Some info:", value="More info", inline=False)
    embed.set_footer(text="Bot designed and coded by IAJ")
    await ctx.send(embed=embed)

@bot.command() #Old test, remove if needed
...
async def status(ctx):
  ...
  for name in names:
      site = 'https://{}.ml'.format(name) # Edit .ml to change site extention
...
async def help(ctx):
    """Help."""
    embed=discord.Embed(title="IAJ's Bot", color=0x800080)
    embed.set_thumbnail(url="https://cdn.discordapp.com/app-icons/789961691894644756/b135a9befb6ea1dfb746673731764a94.png?size=64")
    ...
    miles=discord.Embed(title="IAJ's People Commands", color=0x800080)
    ...
    miles.add_field(name="MP3", value="MP3", inline=False)
    miles.add_field(name="MP4", value="MP4", inline=False)
    miles.add_field(name="Image", value="Picture", inline=False)
...
@bot.command()
async def pages(ctx): # test Feel free to change and use

    contents = ["Page 1", "This is page 2!", "This is page 3!", "This is page 4!"]
    pages = 4
    cur_page = 1
...
async def staff(ctx):
    ...
    embed.add_field(name="Staff", value="Someone", inline=False)
    embed.set_footer(text="Bot designed and coded by IAJ")
...
async def links(ctx):
    ...
    embed.add_field(name="Discord Link:", value="https://discord.alphoneclient.ml", inline=False)
